...
**Answer**
last_name is a chr column / data type.
...
# say the name of our data set and then tell R that we are going to do SOMETHING to/with it
maryland_winred_contributions |> 
# group together all the identical values in the data column, and then do SOMETHING else with the data
  group_by(date) |>
# let's do SOMETHING with those groupings:
  summarise(
    # create a new column called count_contribs that equals the number of rows that have the same date value
    count_contribs = n()
  )
```

Describe what you think this code is doing below. What does the answer represent, compared to the results from `head()`? 
# the code is representing the amount of contributions/donations given on certain dates
...
maryland_winred_contributions |>
  ...
  # arranging the data in descending order (largest to smallest) by the values in count_contribs
...
**Answer**
# summarizing how many rows/donations were made to campaign committee, total sum given to committee, for each comittee we know how much contributions were made/what the total $ amount is
...
**Answer**
# mean and median amounts donated, average amounts of donations 
...
# organizing by cities and total amount, how many contributions (max,min,etc.) were made in each city
