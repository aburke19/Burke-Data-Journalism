...
**ANSWER:** I think the code is displaying the percent of election day and non-election day votes per candidate.
...
**ANSWER:** yes, two representatives had 100% election day percentage rather than the first task, where the highest was 83%
...
**ANSWER:** I think it is useful to see which candidates did not get any percent of the votes, which was 3 different candidates. This could be useful as a journalist to look more into why this happened. You can also see which votes were or were not on election day, which could be interesting info to take into account.
...
**ANSWER:** str_to_lower() converts uppercase to lowercase, str_to_title() makes the first letter of each word capitalized and the rest lowercase
...
**ANSWER:** This code gives us a lot of information, including employers, occupations, where they are from etc. 

```{r}
maryland_winred_categories <- maryland_winred_contributions |>
  #create a new column called amount_category, and what goes in that is...
  mutate(
...
**ANSWER:**
Less than $100 has the most in terms of total dollar amount
...
**ANSWER:** This is important because this new variable only tells us the specific information we want to know about these five categories.
...
**ANSWER:** In expenditure_data, the dates went from random dates, to expeditures on the first of each month.
...
**ANSWER:** The data is showing the months in order but not the years, which I find interesting.
