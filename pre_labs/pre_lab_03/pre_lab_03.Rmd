...
**Answer**The warning says "one or more parsing issues, call `problems()` on your data frame for details, e.g.: dat <- vroom(...) problems(dat)
...
**Answer**9 of the rows are showing errors with N/A. I think the issue is that the code is not specific enough, and doesn't give enough info to show what the data should look like.
...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer**This one does not show any parsing error message.
...
**Answer**Precinct is represented by Box 1, Box 2, etc. District is mostly not featured, showing N/A. Mail is also mostly represented by N/A.
...
```{r}
#update is texas_precinct_20 data so that we change the value in the election_day column so that when election_day is 5, we turn it into just 5. and we make sure that all the other values in election_day that aren't 5 stay the same
...
```{r}
texas_precinct_20 <- texas_precinct_20 |>
  #use mutate to update the election_day column so that it's a numeric column instead
...
We've got some problems again. what are they? One or more parsing issues, call `problems()` on your data frame for
details, e.g.:
  dat <- vroom(...)
  problems(dat)
...
**Answer** I think that the amount of people included in legacy data is interesting
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** there is an amount of donations received, though some seem to look strange as only 6 cents was donated by some
...
**Answer** there is no data included for prefix, suffix and middle name, which can make it hard to answer some questions.
...
**Answer** under 1_linenumber, it says SA11AI. 
...
**Answer**Error in `summarise()`:
ℹ In argument: `total_amount = sum(amount)`.
Caused by error in `sum()`:
I think that this error means that it is not able to summerize the data included and it needs to be more specific.
...
cleaned_conowingo <- conowingo |>
  clean_names() |> 
  #standardizes data, capitalizes or lowercases column names
  rename(linenumber = x1_linenumber) |> #renames column name
  mutate(amount = as.numeric(amount))
#use mutate to update the existing amount column, use as.numeric to turn the values in the existing amount column into actual numbers
distinct() |>
  
  mutate(zip = str_sub(zip, start=11, end=5L))
...
**Answer** There are two, both by Derrick Hamilton
...
**Answer** there is one misspelling, conowingoo
...
**Answer** the misspellings have been fixed
